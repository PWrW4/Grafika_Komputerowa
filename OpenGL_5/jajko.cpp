...
constexpr int den = 20;

static GLfloat viewer[] = { 0.0, 0.0, 15.0 };
// inicjalizacja po�o�enia obserwatora

point3 pointsColor[den*den];
...
void Egg()
{
	...
	if (model == 1) {
		...
		glVertex3fv(pointCloud[9 + 4 * density]);
		glVertex3fv(pointCloud[0 + 6 * density]);
		glVertex3fv(pointCloud[9 + 6 * density]);
		glVertex3fv(pointCloud[0 + 4 * density]);

		glVertex3fv(pointCloud[9 + 7 * density]);
		glVertex3fv(pointCloud[0 + 3 * density]);

		glVertex3fv(pointCloud[9 + 8 * density]);
		glVertex3fv(pointCloud[0 + 2 * density]);
		...
		/*for (int i = 0; i < density*density; i++) {
			...
			glVertex3fv(pointCloud[i]);
			glVertex3fv(pointCloud[i+1]);

			glVertex3fv(pointCloud[i]);
			glVertex3fv(pointCloud[i + density]);

			glVertex3fv(pointCloud[i]);
			glVertex3fv(pointCloud[i + density + 1]);
	...
	else
	{
		...
		//glVertex3fv(pointCloud[0 + 1 * density]);
		//glVertex3fv(pointCloud[0 + 2 * density]);
		//glVertex3fv(pointCloud[1 + 1 * density]);

		//glVertex3fv(pointCloud[1 + 2 * density]);
		//glVertex3fv(pointCloud[0 + 2 * density]);
		//glVertex3fv(pointCloud[1 + 1 * density]);
...
void RenderScene(void)
{
	...
	//if(theta_y >= 0)
	//	if(theta_y > M_PI / 2 && theta_y < M_PI * 3 /2)
		//	up = -1.0;
